...
# Приводим пути к абсолютным ДО смены каталога: иначе относительный
# путь к файлу сломается после cd, а результат уедет не туда.
INPUT="$(realpath -- "$INPUT")"
OUTPUT="$PWD/$(basename "${INPUT%.*}").txt"
